...
public class PurchaseApiController implements PurchaseApi {
  ...
  public ResponseEntity<PurchaseResponse> makePurchase(String cartId, String storeId, Card card) {
    ...
    if (cart == null) {
      return new ResponseEntity<PurchaseResponse>(new PurchaseResponse(Message.CART_NOT_FOUND), HttpStatus.NOT_FOUND);
    }
    if (card.getExpMonth() < 1 || card.getExpMonth() > 12) {
      return new ResponseEntity<PurchaseResponse>(new PurchaseResponse(Message.INVALID_EXP_MONTH),
          HttpStatus.BAD_REQUEST);
    }
    Receipt receipt = purchaseService.makeReceipt(cart, card);
    if (receipt == null) {
      return new ResponseEntity<PurchaseResponse>(new PurchaseResponse(Message.CARD_EXPIRED), HttpStatus.BAD_REQUEST);
    }
